...
export default function RemixActivity(props) {
    ...
    const updateTimeline = () => {
        let newTimelineItems = remix.activity.map((activity, key) => {
            ...
            if (activity.event == "CollectiblePurchased") return <Timeline.Item key={key} color="green" dot={<DollarOutlined />}> <p>Collectible purchased for {parseInt(activity.args.amount_hex, 16)} by:</p>  <p>{activity.args.buyer}</p></Timeline.Item>
            if (activity.event == "RMXPurchased") return <Timeline.Item key={key} color="yellow" dot={<DollarOutlined />}> <p>RMX pruchased for {parseInt(activity.args.amount_hex, 16)} by:</p> <p> {activity.args.buyer}</p></Timeline.Item>
